...
(defn- main-html [config rt]
  ...
  [:html {:lang "en"}
   [:head
    ...
    [:link {:rel "stylesheet" :href (domain/api-path-for (hf/domain rt) :public/static-resource :build (:git/describe config) :resource-name "browser.css")}]
   ...
   [:body
    ...
    (when (not (-> rt hf/domain hf/environment :domain/disable-javascript))
      (concat
        ...
        [[:script {:id "preamble" :src (domain/api-path-for (hf/domain rt) :public/static-resource :build (:git/describe config) :resource-name "browser.js")}]
         [:script {:id "shared" :src (domain/api-path-for (hf/domain rt) :public/static-resource :build (:git/describe config) :resource-name "shared.js")}]
         [:script {:id "main" :src (domain/api-path-for (hf/domain rt) :public/static-resource :build (:git/describe config) :resource-name "main.js")}]]))]])
