(ns hyperfiddle.spec.inference4
...
(defn infer*
  ...
  (cond
    ...
    (keys? v)         (let [qualify (partial qualify (:qualify opts))]
                        [{:type :keys
                          :keys (->> (keys v) (map qualify) #_(filter qualified?) (set))}
                         (reduce (fn [reg [k v]]
                                   (let [[t reg'] (infer* h ps v reg opts)]
                                     (merge-with merge-types reg reg' {k t})))
                                 reg
                                 v)])
    (map? v)          (let [[reg-a keys] (infer* h ps (keys v) reg opts)
                            [reg-b vals] (infer* h ps (keys v) reg opts)]
                        [{:type :map
                          :keys (:child keys)
                          :vals (:child vals)}
                         (merge reg-a reg-b)])
