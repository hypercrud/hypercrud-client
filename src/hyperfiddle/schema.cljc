...
(defn hydrate-schemas [io config pid local-basis partitions]
  (let [dbnames  (-> (:databases config) keys vec)
        requests (mapv (fn [dbname] `((query-schema ~dbname) ~pid))
                       dbnames)]
    (-> (io/hydrate-requests io local-basis partitions requests)
...
(defn -summon-schema-out-of-band
  ...
  [config db-name $]
  (let [result ((hyperfiddle.io.datomic.core/qf (:databases config)
                                                [[db-name nil]])
