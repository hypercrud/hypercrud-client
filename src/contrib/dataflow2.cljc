(ns contrib.dataflow2
  {:clj-kondo/config '{:lint-as {contrib.klass/deftype+ clojure.core/deftype}}}
  (:refer-clojure :exclude [update into val count apply get aget])
  (:require [clojure.string :as str]
            [contrib.array :as arr :refer [length aget apush acopy]]
            [contrib.klass :refer [deftype+]]))
...
(declare get-val get-ended get-error get-frame set-frame set-lock set-next set-prev)
          ...
          (do
            (cond
              ...
              ,, (condp = (type def)
                   ...
                   nil)
              (some #(get-ended %) on) ; if my inbound edges are ended, end me
              ,, (put this (->End)))
...
;; sugar
(defn fmap [f mv] (apply [mv] f))
(defn fapply [f & mvs] (apply mvs f))
