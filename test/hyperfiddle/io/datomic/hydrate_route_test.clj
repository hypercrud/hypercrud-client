...
(def test-config {:fiddle-dbname "$src"
                  :databases     {"$"    {:database/uri (->URI (str "datomic:mem://" 'hyperfiddle.io.datomic.hydrate-route-test "$"))}
  ...
  (testing "non source db"
    (let [pid      hf/root-pid
          response (timbre/with-config {:enabled? false}
                     (let [local-basis (datomic-sync/sync test-config ["$" "$src"])
                           route       `(:persons)
                           partitions  {hf/root-pid {:is-branched true
                                                     :stage       {"$" [[:db/add [:person/name "Bob"] :person/age 41]
                                                                        [:db/add [:person/name "Bob"] :person/age 42]]}}}
                           subject     nil]
                       (hydrate-route/hydrate-route test-config local-basis route pid partitions subject)))]
  ...
  (testing "source db"
    (let [response+ (timbre/with-config {:enabled? false}
                      (let [local-basis (datomic-sync/sync test-config ["$" "$src"])
                            ...
                            subject     nil]
                        @(promise/branch (p/do! (hydrate-route/hydrate-route test-config local-basis route pid partitions subject))
                                         exception/success
                                         exception/failure)))]
      ; issue #1026
